...
@dataclass
class Note:
    # Represents an Anki Note containing shared fields and tags
    nid: int
    ...
    def tags_string(self) -> str:
        # Formats the tags back into Anki's
        return f" {' '.join(sorted(self.tags))} " if self.tags else ""


@dataclass
class Card:
    # Represents Anki Card
    cid: int
    nid: int
    did: int
    deck_name: str = ""
    ...
    def fail_rate(self) -> float:
        # Calculates card failure rate percentage ratio
        if self.reviews == 0:
            return 0.0
    ...
    def avg_time_sec(self) -> float:
        # Converts SQLite millisecond review logs
